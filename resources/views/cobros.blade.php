                ...
                <div class="card">
                    <div class="p-2 mb-3">
                        ...
                        <div class="input-group">
                            <input type="text" v-model="txtbuscar" tabindex="1" @keyup.enter="buscar(false)" class="form-control"
                                placeholder="Buscar...." />
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                ...
                                                <button class='dropdown-item text-primary' @click="showCuotas(c.nro_fact_ventas)"><span
                    ...
                    <div class="card-footer">
                        <div class="form-inline">
                            TOTAL COBRADO: &nbsp;<input type="text" :value="format(cobro.cobrado)" disabled class="form-control form-control-sm text-success font-weight-bold text-monospace"> &nbsp; SALDO:&nbsp; <input type="text" :value="format(cobro.saldo)" disabled class="form-control form-control-sm text-danger font-weight-bold text-monospace">
                        </div>
                    </div>
                ...
                <div class="card p-2">
                        ...
                        <div class="text-secondary text-center">
                            <span class="badge badge-default">
                                ...
                                <span class="badge badge-pill " :class="[ caja.estado=='ABIERTA' ? 'badge-success pr-2 pl-2' : 'badge-danger' ]">  @{{caja.estado}} </span> 
                                |<span class="badge badge-default">
                                    <span class="fa fa-info-circle"></span> #
                                </span>
                                <span class="badge badge-pill " :class="[ caja.estado=='ABIERTA' ? 'badge-success' : 'badge-danger' ]">  @{{caja.nrooperacion}} </span>
                        </div>
                        <hr>
                        <div class="form-group">
                            <label for="fecha">Fecha</label>
                            <input type="date" id="fecha" class="form-control form-control-sm" v-model="cobro.fecha" placeholder="Fecha">
                        </div>
                        ...
                        <div class="d-flex justify-content-center">
                            ...
                            <button class="btn btn-app bg-secondary " @click="cancelar"> <span class="fa fa-times"></span> CANCELAR</button>
                        </div>
...
<script>
    var app= new Vue({
        ...
        methods:{
            ...
            cancelar: function(){
                this.cliente =  {id : 0,documento: 0, nombre: '.-'};
                this.ctas = [];
                this.cobro = {total: 0, saldo: 0, saldonuevo: 0, cobrado: 0, entrega: 0, fecha: '', idSucursal: 0, totalInteres: 0};
            ...
            finalizar: function(){
                axios.post('cobro',{cuotas: this.cuotasAcobrar, cobro : this.cobro})
                .then( response => {
                    if(response.data > 0){
            ...
            addCuota: function(){
                for(i= 0 ; i< this.cuotas.length ; i++){
                    if(this.cuotas[i].check){
                        let validar = this.cuotasAcobrar.findIndex(x=> x.nro_cuotas == this.cuotas[i].nro_cuotas && x.nro_fact_ventas == this.cuotas[i].nro_fact_ventas);
                        if(validar == -1) {
                            let c = {
                                estado: this.cuotas[i].estado, 
                                fecha_venc:this.cuotas[i].fecha_venc,
                                interes: this.setMontoInteres(this.cuotas[i].fecha_venc,this.cuotas[i].monto_cuota), 
                                monto_cobrado: this.cuotas[i].monto_cobrado, 
                                monto_cuota: this.cuotas[i].monto_cuota, 
                                monto_saldo:this.cuotas[i].monto_saldo, 
                                nro_cuotas: this.cuotas[i].nro_cuotas, 
                                nro_fact_ventas: this.cuotas[i].nro_fact_ventas, 
                                acobrar : this.cuotas[i].monto_cuota}
            ...
            getFecha: function() {
                ...
                this.cobro.fecha= f.getFullYear() + "-" + mes.toString().padStart(2, "0") + "-" + dia.toString().padStart(2, "0");
                //this.filtrovalue= this.meses[mes];
            },
            checkCuota: function(index){
                const check = document.getElementById('check'+index).checked;
                if(!check){
                   let validar = this.cuotasAcobrar.findIndex(x=> x.nro_cuotas == this.cuotas[index].nro_cuotas && x.nro_fact_ventas == this.cuotas[index].nro_fact_ventas);
                   if(validar > -1 ) {
            ...
            totalCobrar: function(){
                ...
                if(this.cuotasAcobrar.length > 0){
                    for(i= 0; i< this.cuotasAcobrar.length; i++){
                        this.cobro.total += parseInt(this.cuotasAcobrar[i].monto_saldo) + parseInt(this.cuotasAcobrar[i].interes);
